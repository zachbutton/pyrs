...
- **Ls** displays the pyramid hierarchy as a tree. Optionally includes one-sentence descriptions from each pyramid file.
- **Help** displays the full command reference showing all available pyramid commands, their syntax, and argument conventions.
...
- Children:
  - [Ls](./ls/index.md) — display the pyramid hierarchy
  - [Help](./help/index.md) — display the command reference

## Constraints

- Utility commands must not modify pyramids or code under any circumstances.
- Utility commands must not interpret or analyze content beyond what is needed for display.
