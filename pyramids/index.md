...
This repos code path: ../skills/
- This repo is a set of skill .md files, and does NOT contain tests.
...
- **Alignment is verifiable.** Commands let users check pyramid-to-pyramid alignment (sane), pyramid-to-code alignment (diff), and structural completeness (scan) at any time. Drift is surfaced, not silently resolved.
- **Diff reports are actionable work queues.** A `diff.md` sidecar means unresolved pyramid-to-code gaps still need reconciliation; clean alignment is represented by sidecar absence.

## Contracts

- Every pyramid links to its parent and is referenced by its parent. Orphans are invalid.
- All code changes flow through pyramids: describe the change conceptually first, then apply. Ingest is the sole exception.
...
- Verification artifacts stay actionable: `diff.md` sidecars exist only while discrepancies remain and are removed once resolved.

## Relationships

- Children:
  - [Pyramid Model](./pyramid-model/index.md) — what pyramids are and how they're structured
  - [Change Flow](./change-flow/index.md) — the pyramid-first discipline governing how changes happen
  - [Commands](./commands/index.md) — the command vocabulary users interact with
  - [Distribution](./distribution/index.md) — multi-platform plugin packaging
